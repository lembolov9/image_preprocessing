...
def vis_imgs(X, y, path):
    ...
    if y.ndim == 2:
        y = y[:,:,np.newaxis]
    assert X.ndim == 3
    tl.vis.save_images(np.asarray([X[:,:,0,np.newaxis], y]), size=(1, 2),
        image_path=path)
...
def load_data(path):
    ...
    X_input = np.transpose(np.asarray([np.concatenate(X_input, axis=0)]), (1, 2, 3, 0))
    X_target = np.transpose(np.asarray([np.concatenate(X_target, axis=0)]), (1, 2, 3, 0))
